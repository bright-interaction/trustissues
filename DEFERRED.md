...
## Re-ownership stopped de-duplicating names when 00040 encrypted them

Found while sealing the secret name in `activity_log.detail`, and NOT fixed
there because the fix belongs inside the authority-gated transfer.

`reassignCollectionEntries` (offboard, hard delete of a user) hands the leaver's
shared entries to the deleting admin one at a time, and has a branch for the
case where the new owner already holds an entry by that name: rename the
incoming one to `"<name> (from <leaver>)"` and retry, so the team keeps both.
00040 broke that branch in two independent ways.

- **It is unreachable.** The collision it catches was `UNIQUE(user_id, name)`.
  00040 made that constraint vacuous on purpose (a fresh nonce per seal means
  two equal names no longer produce equal ciphertext) and moved the real
  constraint to the partial unique index over `name_bidx`. That index is keyed
  by `user_id`, and `TransferSecretOwnership` writes `user_id` WITHOUT
  recomputing `name_bidx`. So the transfer no longer collides at all: it
  succeeds, and the new owner is left holding two entries with the same name,
  one of them carrying a blind index token computed under the previous owner.
  The stale token is the worse half, because it means per-user name uniqueness
  is no longer enforced for that row at all.
- **If it were reachable it would corrupt the row.** `dedup` concatenates the
  suffix onto `row.Name`, which is ciphertext since 00040, and
  `RenameVaultEntry` writes the result straight back. The column would then hold
  `enc:v1:<base64> (from someone@example.com)`, which is not valid ciphertext
  and which no key can ever open. `RenameVaultEntry` also does not write
  `name_bidx`, which `UpdateVaultEntryName` documents as the thing a rename must
  never skip.

**The shape of the fix.** `TransferSecretOwnership` has to carry the new
`name_bidx`, recomputed under the new custodian from the opened name, so the
index keeps meaning what it says across a change of `user_id`. Once it does, the
collision fires again and the dedup branch has to build its new name from the
OPENED name, seal it, and write both columns through `UpdateVaultEntryName`.
That is a `vaultegress` query change plus its proof plumbing and a regenerated
`sqlc`, which is why it is here rather than in the pass that found it.

**Blast radius today.** Only the hard-delete-a-user path, and only for entries
in shared collections. Nothing is lost or exposed: the entries transfer and stay
readable. What is gone is the uniqueness guarantee for transferred rows and the
"keep both, renamed" behaviour the confirmation dialog implies.
