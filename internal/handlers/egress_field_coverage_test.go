...
var vaultEntryEgressClass = map[string]struct {
	...
	"name_bidx": {egressNeutral, "blind index over name, carrying the per-user uniqueness the " +
		"inline UNIQUE(user_id, name) can no longer enforce over ciphertext. It names no host and " +
		"triggers no delivery; it decides only whether a second entry may take the same label"},
