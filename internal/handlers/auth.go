...
func decryptTOTPSecret(stored string, keys ...string) string {
	...
	if dec, err := columncrypto.DecryptStringAny(stored, keys...); err == nil {
		return dec
	}
	return stored
}
	...
	for _, row := range rows {
		...
		if columncrypto.IsEncrypted(stored) {
			if _, derr := columncrypto.DecryptString(stored, h.cfg.VaultKey); derr != nil {
				if _, prevErr := columncrypto.DecryptStringAny(stored, h.cfg.VaultKeyPrevious); prevErr == nil {
		...
		seed := stored
		if dec, derr := columncrypto.DecryptStringAny(stored, h.cfg.VaultKey, h.cfg.VaultKeyPrevious); derr == nil {
			seed = dec
		}
