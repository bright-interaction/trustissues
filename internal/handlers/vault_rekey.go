...
var rekeySurfaces = []keyedSurface{
	...
	{Table: "vault_entries", Column: "url", Family: familyVaultColumn,
		Why: "which site a credential belongs to"},
	{Table: "vault_entries", Column: "alias_url", Family: familyVaultColumn,
		Why: "a second host the same credential logs into"},
	{Table: "vault_entries", Column: "username", Family: familyVaultColumn,
		Why: "the account name the secret pairs with"},
	{Table: "vault_entries", Column: "category", Family: familyVaultColumn,
		Why: "user-chosen grouping label"},
	{Table: "vault_entries", Column: "notes", Family: familyVaultColumn,
		Why: "free text, routinely holds recovery codes and second factors"},
	{Table: "vault_entries", Column: "provider_meta", Family: familyVaultColumn,
		Why: "provider account ids and key ids used by auto-rotation"},
	{Table: "vault_entries", Column: "rotation_targets", Family: familyVaultColumn,
		Why: "delivery endpoints, embeds webhook HMAC secrets"},
	{Table: "vault_entries", Column: "custom_fields", Family: familyVaultColumn,
	...
	{Table: "invitations", Column: "code", Family: familyVaultColumn,
		Why: "the pending invite code, kept recoverable so it can be re-sent"},
	{Table: "users", Column: "totp_secret", Family: familyColumnCrypto,
...
type rekeyPlan struct {
	entries  []db.RekeyVaultEntryParams
...
func (h *VaultHandler) applyPlan(ctx context.Context, q *db.Queries, plan *rekeyPlan) error {
	for _, p := range plan.entries {
		if err := q.RekeyVaultEntry(ctx, p); err != nil {
			return fmt.Errorf("rekey vault entry %s: %w", p.ID, err)
		}
	}
...
func (h *VaultHandler) classifyVaultColumn(stored string) (plain string, age keyAge) {
	if stored == "" {
		return "", keyAgeEmpty
	}
	...
	plain, onPrevious, err := h.decryptColumnWithKeyAge(stored)
	if err != nil {
...
// sweep's problem.
func (h *VaultHandler) classifyColumnCrypto(stored string) (plain string, age keyAge) {
	if stored == "" {
		return "", keyAgeEmpty
	}
	if dec, err := columncrypto.DecryptString(stored, h.keySource); err == nil {
		return dec, keyAgeCurrent
	}
	if h.previous != nil {
		if dec, err := columncrypto.DecryptString(stored, h.previous.source); err == nil {
...
// classifyVaultValue classifies a raw AES-GCM value (the vault secret, or a
// notification channel config) at whatever derivation version it carries, and
// returns the plaintext when it opened.
//
// The caller MUST zero the returned plaintext once it has been re-encrypted.
//
...
// cleartext, which the length check above already caught.
func (h *VaultHandler) classifyVaultValue(ciphertext, nonce []byte, encVersion int) (plain []byte, age keyAge) {
	if len(ciphertext) == 0 || len(nonce) == 0 {
		return nil, keyAgeEmpty
	}
	if encVersion == 1 {
		if pt, err := decryptWithKey(h.legacyKeyFor(h.keySource), ciphertext, nonce); err == nil {
			return pt, keyAgeCurrent
		}
		if h.previous != nil {
			if pt, err := decryptWithKey(h.legacyKeyFor(h.previous.source), ciphertext, nonce); err == nil {
				return pt, keyAgePrevious
			}
		}
		return nil, keyAgeUnknown
	}
	if pt, err := decryptWithKey(h.encryptionKey, ciphertext, nonce); err == nil {
		return pt, keyAgeCurrent
	}
	if h.previous != nil {
		if pt, err := decryptWithKey(h.previous.value, ciphertext, nonce); err == nil {
			return pt, keyAgePrevious
		}
	}
	return nil, keyAgeUnknown
...
func (h *VaultHandler) scanVaultEntries(ctx context.Context, q *db.Queries, sc *scanCtx, plan *rekeyPlan) error {
	...
	for _, row := range rows {
		...
		plain, age := h.classifyVaultValue(row.EncryptedValue, row.Nonce, encVersion)
		...
		cols := []metaCol{
			{"url", nullStringToString(row.Url)},
			{"alias_url", nullStringToString(row.AliasUrl)},
			{"username", nullStringToString(row.Username)},
			{"category", nullStringToString(row.Category)},
			{"notes", nullStringToString(row.Notes)},
			{"provider_meta", nullStringToString(row.ProviderMeta)},
			{"rotation_targets", nullStringToString(row.RotationTargets)},
			{"custom_fields", row.CustomFields},
		}
		newCols := make(map[string]string, len(cols))
		plains := make(map[string]string, len(cols))
		for _, c := range cols {
			p, a := h.classifyVaultColumn(c.stored)
		...
		plan.entries = append(plan.entries, db.RekeyVaultEntryParams{
...
func (h *VaultHandler) scanInvitations(ctx context.Context, q *db.Queries, sc *scanCtx, plan *rekeyPlan) error {
	...
	for _, row := range rows {
		plain, age := h.classifyVaultColumn(row.Code)
...
func (h *VaultHandler) scanTOTPSecrets(ctx context.Context, q *db.Queries, sc *scanCtx, plan *rekeyPlan) error {
	...
	for _, row := range rows {
		...
		seed, age := h.classifyColumnCrypto(stored)
...
func (h *VaultHandler) scanSettings(ctx context.Context, q *db.Queries, sc *scanCtx, plan *rekeyPlan) error {
	for _, s := range rekeySurfaces {
		...
		plain, age := h.classifyColumnCrypto(stored)
