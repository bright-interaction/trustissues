...
type DeleteUserIfNotLastAdminParams struct {
	ID             string      `json:"id"`
...
func (q *Queries) DeleteUserIfNotLastAdmin(ctx context.Context, arg DeleteUserIfNotLastAdminParams) (sql.Result, error) {
	return q.db.ExecContext(ctx, deleteUserIfNotLastAdmin, arg.ID, arg.PrivateIngress)
}
...
const setInitialPasswordHash = `-- name: SetInitialPasswordHash :execresult
UPDATE users
SET password_hash = ?, password_set = 1, updated_at = CURRENT_TIMESTAMP
WHERE id = ? AND password_set = 0
`

type SetInitialPasswordHashParams struct {
	PasswordHash string `json:"password_hash"`
	ID           string `json:"id"`
}
...
func (q *Queries) SetInitialPasswordHash(ctx context.Context, arg SetInitialPasswordHashParams) (sql.Result, error) {
	return q.db.ExecContext(ctx, setInitialPasswordHash, arg.PasswordHash, arg.ID)
}
...
const userHasProtectedCollectionImpact = `-- name: UserHasProtectedCollectionImpact :one
SELECT EXISTS (
  SELECT 1
  FROM collection_members cm
  JOIN collections c ON c.id = cm.collection_id
  WHERE cm.user_id = ?1 AND c.private_access_policy != 'standard'
  UNION ALL
  SELECT 1
  FROM vault_entries e
  JOIN collections c ON c.id = e.collection_id
  WHERE e.user_id = ?1 AND c.private_access_policy != 'standard'
)
`
...
func (q *Queries) UserHasProtectedCollectionImpact(ctx context.Context, userID string) (bool, error) {
	row := q.db.QueryRowContext(ctx, userHasProtectedCollectionImpact, userID)
	var exists bool
	err := row.Scan(&exists)
	return exists, err
}
