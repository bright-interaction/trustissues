...
--
-- It is deliberately NOT replaced with `WHERE name_bidx = ?`. A pre-backfill,
-- interrupted, or manually damaged legacy row can still carry an empty/stale
-- token even though the boot repair now converges ordinary same-scope
-- collisions by assigning a deterministic duplicate label. A bidx-only lookup
-- would silently strand such a row until the next successful repair. The name
-- is matched in Go instead, on the decrypted value, so lookup remains compatible
-- with both repaired and not-yet-repaired rows.
--
