...
func (q *Queries) ListUsersWithEntryCount(ctx context.Context, privateIngress interface{}) ([]ListUsersWithEntryCountRow, error) {
	rows, err := q.db.QueryContext(ctx, listUsersWithEntryCount, privateIngress)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
