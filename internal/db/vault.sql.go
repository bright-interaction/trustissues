...
func (q *Queries) ListCollectionVaultEntriesForUser(ctx context.Context, userID string) ([]ListCollectionVaultEntriesForUserRow, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&i.ID, &i.Name, &i.CollectionID); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
...
const listCollectionVaultEntryNames = `-- name: ListCollectionVaultEntryNames :many
SELECT id, name FROM vault_entries WHERE collection_id = ?
`

type ListCollectionVaultEntryNamesRow struct {
	ID   string `json:"id"`
	Name string `json:"name"`
}
...
func (q *Queries) ListCollectionVaultEntryNames(ctx context.Context, collectionID sql.NullString) ([]ListCollectionVaultEntryNamesRow, error) {
	rows, err := q.db.QueryContext(ctx, listCollectionVaultEntryNames, collectionID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListCollectionVaultEntryNamesRow{}
	for rows.Next() {
		var i ListCollectionVaultEntryNamesRow
...
const listPersonalVaultEntryNames = `-- name: ListPersonalVaultEntryNames :many
SELECT id, name FROM vault_entries
WHERE user_id = ? AND (collection_id IS NULL OR collection_id = '')
`

type ListPersonalVaultEntryNamesRow struct {
	ID   string `json:"id"`
	Name string `json:"name"`
}
...
func (q *Queries) ListPersonalVaultEntryNames(ctx context.Context, userID string) ([]ListPersonalVaultEntryNamesRow, error) {
	rows, err := q.db.QueryContext(ctx, listPersonalVaultEntryNames, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListPersonalVaultEntryNamesRow{}
	for rows.Next() {
		var i ListPersonalVaultEntryNamesRow
		if err := rows.Scan(&i.ID, &i.Name); err != nil {
			return nil, err
		}
		items = append(items, i)
...
type ResolveVaultReferenceRow struct {
	...
	EncryptedValue      []byte         `json:"encrypted_value"`
	Nonce               []byte         `json:"nonce"`
	PrivateAccessPolicy sql.NullString `json:"private_access_policy"`
}
